...
test('task prompt accepts the follow-up variant and an optional support section', () => {
...
test('task prompt rejects unsupported variants and incomplete visible sections', () => {
  assert.throws(() => normalizeTaskPrompt({ variant: 'yourTurn', title: 'Title', text: 'Text' }), /supported variant/);
  assert.throws(() => normalizeTaskPrompt({ variant: 'followUp', title: '', text: 'Text' }), /non-empty/);
...
test('task prompt styles derive variants and divider from component structure', () => {
  ...
  assert.doesNotMatch(css, /\.task-prompt--your-turn/);
