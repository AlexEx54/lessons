...
test('live class: guest authorization, actions, isolation, tab replacement and restart recovery', { timeout: 20000 }, async t => {
  ...
  await teacherSocket.next('pointer');
  await studentSocket.next('pointer');
  const pointerAction = { type: 'pointer-set', stageId: 'warm-up', target: { componentId: choice.id, part: null } };
  studentSocket.send(JSON.stringify(pointerAction));
  assert.match((await studentSocket.next('pointer-error')).error, /преподаватель/);
  teacherSocket.send(JSON.stringify({ ...pointerAction, target: { componentId: 'private-note', part: null } }));
  assert.match((await teacherSocket.next('pointer-error')).error, /недоступен/);
  teacherSocket.send(JSON.stringify({ ...pointerAction, stageId: 'reading' }));
  assert.match((await teacherSocket.next('pointer-error')).error, /недоступен/);
  teacherSocket.send(JSON.stringify(pointerAction));
  const pointed = await teacherSocket.next('pointer');
  assert.deepEqual((await studentSocket.next('pointer')), pointed);
  assert.deepEqual(pointed.target, pointerAction.target);
  teacherSocket.send(JSON.stringify({ type: 'pointer-clear' }));
  assert.equal((await teacherSocket.next('pointer')).target, null);
  assert.equal((await studentSocket.next('pointer')).target, null);
  teacherSocket.send(JSON.stringify(pointerAction));
  await teacherSocket.next('pointer');
  await studentSocket.next('pointer');
  ...
  assert.deepEqual((await replacement.next('pointer')).target, pointerAction.target);
  ...
  assert.equal((await teacherSocket.next('pointer')).target, null);
  assert.equal((await replacement.next('pointer')).target, null);
