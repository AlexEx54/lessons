...
test('text panel normalizes text, HEX color, and independent optional pictures', () => {
  ...
  const leading = normalizeTextPanel(panel({
  ...
  const trailing = normalizeTextPanel(panel({ trailingPicture: { imagePrompt: 'Symbol' } }));
  assert.equal(trailing.leadingPicture, null);
  assert.deepEqual(trailing.trailingPicture, { imagePrompt: 'Symbol' });

  const both = normalizeTextPanel(panel({
...
test('text panel rejects invalid ids, empty content, colors, and incomplete pictures', () => {
  ...
  assert.throws(() => normalizeTextPanel(panel({ leadingPicture: {} })), /imagePrompt/);
  assert.throws(() => normalizeTextPanel(panel({ trailingPicture: 'image' })), /must be an object/);
});
