...
test('class API creates once, renders schedule and teacher lesson, protects all routes and supports snapshot audio ranges', async t => {
  ...
  assert.equal(invite.headers.get('location'), lesson.lessonPath);
  assert.equal((await request(lesson.invitePath, otherCookie)).status, 404);
  ...
  assert.equal(legacyInvite.headers.get('location'), lesson.lessonPath);
  assert.equal((await request(`/join/${legacyToken}`, otherCookie)).status, 404);
