...
function readingMessages(topic, vocabularyItems, learnerProfile) {
  const { terms } = readingVocabularyTerms(vocabularyItems);
  return [{
    role: 'system',
    content: [
...
function listeningMessages(topic, vocabularyItems, learnerProfile) {
  const { terms } = readingVocabularyTerms(vocabularyItems);
  return [{
    role: 'system',
    content: [
...
async function generateReading({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: readingMessages(topic, vocabularyItems, { ageGroup, level }),
...
async function generateListening({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: listeningMessages(topic, vocabularyItems, { ageGroup, level }),
