...
const { normalizeGuidedRoleCards } = require('../assets/components/guided-role-cards.js');
const { normalizeHowToPlay } = require('../assets/components/how-to-play.js');
const { normalizeMultipleChoice } = require('../assets/components/multiple-choice.js');
const { normalizeSpeakingSupport } = require('../assets/components/speaking-support.js');
const { normalizeThreeTwoOne } = require('../assets/components/three-two-one.js');
const { normalizeSelfAssessment } = require('../assets/components/self-assessment.js');
...
const DEEPSEEK_FLASH_VISION_MODEL = 'deepseek/deepseek-v4-flash-vision-exp';
const LESSON_MODEL_OPTIONS = Object.freeze({
  [OPENROUTER_MODEL]: 'Gemini 3.7 Flash',
  [DEEPSEEK_FLASH_VISION_MODEL]: 'DeepSeek V4 Flash Vision Exp · High Reasoning',
});
...
const LESSON_METADATA_RESPONSE_SCHEMA = Object.freeze({
  type: 'object',
  additionalProperties: false,
  required: ['coverImagePrompt'],
  properties: {
    coverImagePrompt: { type: 'string', minLength: 1 },
  },
});
...
const GUIDED_ROLE_SCHEMA = Object.freeze({
  type: 'object',
  additionalProperties: false,
  required: ['want', 'avoid', 'secret', 'mission', 'goal'],
  properties: {
    want: {
...
const GUIDED_SUPPORT_SCHEMA = Object.freeze({
  type: 'array', minItems: 3, maxItems: 3,
  items: { type: 'string', minLength: 1 },
});

const GUIDED_SPEAKING_RESPONSE_SCHEMA = Object.freeze({
  type: 'object',
  additionalProperties: false,
  required: ['teacherNotes', 'usedVocabularyTerms', 'roles', 'speakingSupport', 'dialogue'],
  properties: {
    teacherNotes: { type: 'string', minLength: 1 },
    usedVocabularyTerms: {
    ...
    roles: {
      ...
      properties: {
        student: GUIDED_ROLE_SCHEMA,
        teacher: GUIDED_ROLE_SCHEMA,
      },
    },
    speakingSupport: {
      type: 'object',
      ...
      properties: {
        reacting: GUIDED_SUPPORT_SCHEMA,
        followUpQuestions: GUIDED_SUPPORT_SCHEMA,
        clarification: GUIDED_SUPPORT_SCHEMA,
        suggestions: GUIDED_SUPPORT_SCHEMA,
        agreeingDisagreeing: GUIDED_SUPPORT_SCHEMA,
        decision: GUIDED_SUPPORT_SCHEMA,
      },
    },
    dialogue: {
...
const WRAP_UP_RESPONSE_SCHEMA = Object.freeze({
  type: 'object',
  additionalProperties: false,
  required: [
    'teacherNotes', 'threePrompt', 'twoPrompt', 'twoCues', 'onePrompt', 'possibleLanguage',
  ],
  properties: {
    teacherNotes: {
      type: 'object',
      additionalProperties: false,
      required: ['signsOfSuccess', 'struggleSupport', 'positiveEnding'],
      properties: {
        signsOfSuccess: { type: 'string', minLength: 1 },
        struggleSupport: { type: 'string', minLength: 1 },
        positiveEnding: { type: 'string', minLength: 1 },
      },
    },
    threePrompt: { type: 'string', minLength: 1 },
    twoPrompt: { type: 'string', minLength: 1 },
    twoCues: {
      type: 'array',
      minItems: 2,
      maxItems: 2,
    ...
    onePrompt: { type: 'string', minLength: 1 },
    possibleLanguage: {
      type: 'array',
      minItems: 3,
      maxItems: 3,
...
function createLessonSkeleton(topic, { ageGroup = '12-14', level = 'A2', model = OPENROUTER_MODEL } = {}) {
...
function learnerProfileInstructions({ ageGroup = '12-14', level = 'A2' } = {}) {
  return [
    'Target learner:',
    `- CEFR level: ${level}`,
    `- Age: ${ageGroup}`,
    'Student-facing and teacher-facing lesson language difficulty must follow the CEFR level.',
    'Topics, situations, and tone must be appropriate for the age.',
    'The learner profile does not change image-generation prompts; follow each section’s image requirements.',
  ].join('\n');
}

function buildLessonMetadata(generated) {
  if (!generated || typeof generated !== 'object') {
    throw new Error('Gemini вернул пустые метаданные урока.');
  }
  const coverImagePrompt = String(generated.coverImagePrompt || '').trim();
  if (!coverImagePrompt) {
    throw new Error('Метаданные урока должны содержать промпт для обложки.');
  }
  return { coverImagePrompt };
}

function applyLessonMetadataToSkeleton(skeleton, generated) {
  const lesson = JSON.parse(JSON.stringify(skeleton));
  const metadata = buildLessonMetadata(generated);
  lesson.meta.coverImagePrompt = metadata.coverImagePrompt;
  return lesson;
}
...
function buildTargetVocabularyContent(generated) {
  ...
  if (!Array.isArray(generated.sentenceStarters) || generated.sentenceStarters.length < 1) {
    throw new Error('Target Vocabulary требует sentence starters.');
  }
  const sentenceStarters = generated.sentenceStarters
...
function buildGrammarFocusContent(generated, vocabularyItems, random = Math.random) {
  ...
  if (dialogueTurns.length < 6 || dialogueTurns.length > 8
    ...
    throw new Error('Grammar Focus Task 2 должен содержать 6–8 реплик на отдельных строках с жирными speaker labels.');
  }
...
function buildGuidedSpeakingContent(generated, vocabularyItems) {
  if (!generated || typeof generated !== 'object') {
    throw new Error('Gemini вернул пустой Guided Speaking.');
  }
  if (/[Ѐ-ӿ]/u.test(JSON.stringify(generated))) {
    throw new Error('Guided Speaking должен быть полностью на английском языке.');
  }

  const { lookup } = readingVocabularyTerms(vocabularyItems);
  if (!Array.isArray(generated.usedVocabularyTerms)
    || generated.usedVocabularyTerms.length < 2
    || generated.usedVocabularyTerms.length > 4) {
    throw new Error('Guided Speaking должен использовать от 2 до 4 элементов Target Vocabulary.');
  }
  const usedVocabularyTerms = generated.usedVocabularyTerms.map((value) => {
    const requested = generatedText(value, 'элемент Target Vocabulary в Guided Speaking');
    const matched = lookup.get(requested.toLocaleLowerCase());
    if (!matched) throw new Error('Guided Speaking использует неизвестный элемент Target Vocabulary.');
    return matched;
  });
  assertUnique(usedVocabularyTerms, 'элементы Target Vocabulary в Guided Speaking');

  function list(value, count, label) {
    return exactArray(value, count, label)
      .map(item => generatedText(item, label));
  }

  function role(value, label) {
    if (!value || typeof value !== 'object') {
      throw new Error(`Guided Speaking требует роль ${label}.`);
    }
    return {
      want: list(value.want, 2, `пункта want роли ${label}`),
      avoid: list(value.avoid, 1, `пункт avoid роли ${label}`),
      secret: list(value.secret, 2, `пункта secret роли ${label}`),
      mission: list(value.mission, 2, `пункта mission роли ${label}`),
      goal: generatedText(value.goal, `goal роли ${label}`),
    };
  }

  const studentRole = role(generated.roles?.student, 'Student');
  const teacherRole = role(generated.roles?.teacher, 'Teacher');
  if (studentRole.goal !== teacherRole.goal) {
    throw new Error('Роли Guided Speaking должны иметь одинаковую общую цель.');
  }

  const dialogueItems = Array.isArray(generated.dialogue) ? generated.dialogue : [];
  if (dialogueItems.length < 6 || dialogueItems.length > 10) {
    throw new Error('Example Dialogue Guided Speaking должен содержать от 6 до 10 реплик.');
  }
  const dialogue = dialogueItems.map((item, index) => {
    if (!['Teacher', 'Student'].includes(item?.speaker)) {
      throw new Error('Example Dialogue Guided Speaking допускает только роли Teacher и Student.');
    }
    if (index > 0 && dialogueItems[index - 1]?.speaker === item.speaker) {
      throw new Error('Реплики Example Dialogue Guided Speaking должны чередоваться.');
    }
    return {
      speaker: item.speaker,
      text: generatedText(item.text, 'реплика Example Dialogue'),
    };
  });
  if (new Set(dialogue.map(item => item.speaker)).size !== 2) {
    throw new Error('Example Dialogue Guided Speaking требует реплики Teacher и Student.');
  }

  const vocabularyContext = JSON.stringify({
    roles: generated.roles,
    dialogue: generated.dialogue,
  }).toLocaleLowerCase();
  usedVocabularyTerms.forEach((term) => {
    if (!vocabularyContext.includes(term.toLocaleLowerCase())) {
      throw new Error(`Элемент Target Vocabulary “${term}” отсутствует в ролях и диалоге Guided Speaking.`);
    }
  });
  ...
  const supportDefinitions = {
    ...
    agreeingDisagreeing: 'Agreeing / Disagreeing',
    decision: 'Decision',
  };
  const supportSections = {};
  Object.entries(supportDefinitions).forEach(([key, title]) => {
    supportSections[key] = {
      title,
      text: list(generated.speakingSupport?.[key], 3, `фразы Speaking Support ${title}`)
        .map(item => `- ${item}`).join('\n'),
    };
  });
  const markdownList = items => items.map(item => `- ${item}`).join('\n');
  const roleSections = value => ({
    want: markdownList(value.want),
    avoid: markdownList(value.avoid),
    secret: markdownList(value.secret),
    mission: markdownList(value.mission),
    goal: `- ${value.goal}`,
  });
  ...
  const teacherNote = {
    ...
    text: englishTeacherNotes(generated.teacherNotes, 'Guided Speaking'),
  };
  ...
  const roleCards = {
    ...
    roles: {
      student: { title: 'Student', sections: roleSections(studentRole) },
      teacher: { title: 'Teacher', sections: roleSections(teacherRole) },
    },
  };
  ...
  const exampleDialogue = {
    ...
    accentColor: '#3563D4',
    studentVisibility: 'always',
    text: dialogue.map(item => `**${item.speaker}:** ${item.text}`).join('\n\n'),
  };

  normalizeTeacherNote(teacherNote);
  normalizeTextPanel(readInstructions);
  normalizeHowToPlay(howToPlay);
  normalizeGuidedRoleCards(roleCards);
  normalizeSpeakingSupport(speakingSupport);
  normalizeMarkdownCard(exampleDialogue);
  return [teacherNote, readInstructions, howToPlay, roleCards, speakingSupport, exampleDialogue];
}

function applyGuidedSpeakingToSkeleton(skeleton, generated, vocabularyItems) {
  const lesson = JSON.parse(JSON.stringify(skeleton));
  const stage = lesson.stages.find(candidate => candidate.id === 'guided-speaking');
  if (!stage) throw new Error('В шаблоне отсутствует стадия Guided Speaking.');
  stage.content = buildGuidedSpeakingContent(generated, vocabularyItems);
  return lesson;
}

function wrapUpText(value, label) {
  const normalized = typeof value === 'string' ? value.trim() : '';
  if (!normalized) throw new Error(`Wrap-Up требует ${label}.`);
  return normalized;
}

function wrapUpArray(value, count, label) {
  if (!Array.isArray(value) || value.length !== count) {
    throw new Error(`Wrap-Up требует ровно ${count} ${label}.`);
  }
  return value.map((item, index) => wrapUpText(item, `${label} №${index + 1}`));
}

function assertWrapUpEnglish(value) {
  if (typeof value === 'string' && /[\u0400-\u04FF]/u.test(value)) {
    throw new Error('Wrap-Up должен быть полностью на английском языке.');
  }
  if (Array.isArray(value)) value.forEach(assertWrapUpEnglish);
  else if (value && typeof value === 'object') Object.values(value).forEach(assertWrapUpEnglish);
}

function buildWrapUpContent(generated) {
  if (!generated || typeof generated !== 'object') {
    throw new Error('Gemini вернул пустой Wrap-Up.');
  }
  assertWrapUpEnglish(generated);

  const notes = generated.teacherNotes;
  if (!notes || typeof notes !== 'object' || Array.isArray(notes)) {
    throw new Error('Wrap-Up требует Teacher’s Notes.');
  }
  const signsOfSuccess = wrapUpText(notes.signsOfSuccess, 'признаки успеха');
  const struggleSupport = wrapUpText(notes.struggleSupport, 'помощь при затруднениях');
  const positiveEnding = wrapUpText(notes.positiveEnding, 'позитивное завершение');
  const twoCues = wrapUpArray(generated.twoCues, 2, 'подсказки для шага two');
  const possibleLanguage = wrapUpArray(
    generated.possibleLanguage, 3, 'фразы Possible language',
  );
  ...
  const teacherNote = {
    ...
    text: [
      `- **Signs of success:** ${signsOfSuccess}`,
      `- **If the student struggles:** ${struggleSupport}`,
      `- **Positive ending:** ${positiveEnding}`,
    ].join('\n'),
  };
  const threeTwoOne = {
    type: 'threeTwoOne',
    ...
    steps: {
      three: { prompt: wrapUpText(generated.threePrompt, 'prompt шага three') },
      two: {
        prompt: wrapUpText(generated.twoPrompt, 'prompt шага two'),
        text: twoCues.map((cue, index) => `${index + 1}. ${cue}`).join('\n'),
      },
      one: {
        label: 'Can-do question',
        prompt: wrapUpText(generated.onePrompt, 'prompt шага one'),
      },
    },
  };
  const selfAssessment = { ...WRAP_UP_SELF_ASSESSMENT };
  const languageCard = {
    type: 'markdownCard',
    id: 'wrap-up-possible-language',
    title: 'Possible language:',
    text: possibleLanguage.join(' / '),
    icon: 'chat',
    accentColor: '#6545F5',
    studentVisibility: 'always',
  };
  ...
  normalizeTeacherNote(teacherNote);
  normalizeThreeTwoOne(threeTwoOne);
  normalizeSelfAssessment(selfAssessment);
  normalizeMarkdownCard(languageCard);
  return [teacherNote, threeTwoOne, selfAssessment, languageCard];
}

function applyWrapUpToSkeleton(skeleton, generated) {
  const lesson = JSON.parse(JSON.stringify(skeleton));
  const stage = lesson.stages.find(candidate => candidate.id === 'wrap-up');
  if (!stage) throw new Error('В шаблоне отсутствует стадия Wrap-Up.');
  stage.content = buildWrapUpContent(generated);
  return lesson;
}

function lessonMetadataMessages(topic, learnerProfile) {
  return [{
    role: 'system',
    content: [
      learnerProfileInstructions(learnerProfile),
      'You create visual metadata for one-to-one English lessons.',
      'Create coverImagePrompt for the lesson as a whole, based only on the supplied Lesson topic.',
      'Write a vivid, attractive English image-generation prompt for a landscape 16:10 lesson-library cover that communicates the topic at a glance.',
      'Use a polished contemporary educational illustration style suitable for teenagers, with one clear focal scene and a composition that works as a finished cover without an overlaid title. Avoid childish or preschool aesthetics.',
      'Do not reserve empty space for a title. Explicitly require no visible or readable text, letters, numbers, logos, or watermarks.',
...
function warmUpMessages(topic, learnerProfile) {
  return [{
    role: 'system',
    content: [
      learnerProfileInstructions(learnerProfile),
      'You design one-to-one English lessons.',
      ...
      '1. In English, briefly explain the purpose of the lead-in question, then provide the exact question for the learner in natural English at the target CEFR level; it must summarize the main Warm-Up topic and guide the learner toward the task.',
      ...
      'After the three bullet points, add a blank line and then give the teacher a ready-to-say opening phrase in natural English at the target CEFR level using exactly this Markdown format: **Say:** *English opening phrase*.',
      ...
      'All student-facing copy, captions, questions, and possible language must be in natural English at the target CEFR level. Image prompts must be in English.',
      'Create exactly four engaging This or That pairs with exactly two clearly contrasting options in each pair.',
      'Every imagePrompt must describe a modern square educational illustration suitable for teenagers, with no text. Avoid childish or preschool aesthetics.',
...
function leadInMessages(topic, learnerProfile) {
  return [{
    role: 'system',
    content: [
      learnerProfileInstructions(learnerProfile),
      ...
      '4. Tell the teacher in English to ask after all three answers whether the learner can guess the lesson topic; include a ready-to-say natural English question at the target CEFR level.',
      '5. Finish with a ready-to-say natural English lesson preview at the target CEFR level in the exact Markdown format **Say:** *English lesson preview*.',
      ...
      'Use natural English at the target CEFR level in the message, but include exactly one or two contemporary English phrases, idioms, or slang expressions. Those expressions must be the ones explained in teacherNotes.',
      ...
      'Create exactly three student-facing questions in natural English at the target CEFR level.',
      ...
      'Both image prompts must describe modern educational illustrations suitable for teenagers, with no text. Avoid childish or preschool aesthetics. The leading image should work as a sender/avatar image and the trailing image as a small thematic symbol related to the message.',
...
function targetVocabularyMessages(topic, learnerProfile) {
  return [{
    role: 'system',
    content: [
      learnerProfileInstructions(learnerProfile),
      ...
      'All content must be in natural English at the target CEFR level and directly connected to the supplied lesson topic.',
      ...
      'Create exactly 10 distinct, useful target words or short phrases. Prefer lexical chunks that a learner at the target CEFR level can use when speaking about the topic.',
      'Give every term a short, simple English definition and an English imagePrompt.',
      'Every imagePrompt must explicitly describe a modern square educational illustration suitable for teenagers, with no text. Avoid childish or preschool aesthetics.',
      "The images will appear together in a word-to-picture matching exercise. Their main purpose is to make the correct term recognizable and clearly distinguishable from every other term in this vocabulary set.",
      "Plan all 10 image concepts together before writing the final prompts. Give each term a distinct visual scene with a clear identifying cue. Compare each scene against all other terms: if another term would be an equally reasonable answer, redesign the scene.",
      "Avoid similar scenes for different terms, such as multiple students sitting at desks or multiple calendars. Prefer concrete actions, objects, and observable contrasts over facial expressions, mood, or generic topic-related settings.",
      "Do not rely on invisible properties or intentions, such as a phone being silent, a goal being realistic, or a student feeling motivated. Show the action or contrast that makes the meaning visible.",
      "For abstract terms, use a simple two-panel scene if needed to show repetition, a change, or a contrast. Specify what appears in each panel.",
      "Write each imagePrompt as a self-contained drawing instruction. Describe the main subject, exact action, and identifying visual cue first, then style and background. Do not merely ask the generator to illustrate the term: describe exactly what the viewer should see.",
      "Make the identifying cue large and prominent enough to recognize in a small thumbnail. Use a simple composition with only necessary details. Avoid decorative clutter and do not rely on readable words, labels, numbers, or sentences.",
      "Use a consistent modern educational illustration style across the set. Before returning the result, mentally match each proposed image against the full vocabulary list without its intended label. Revise any scene that lacks a visible identifying cue or fits multiple terms.",
      ...
      'Create short, topic-relevant sentence starters as plain text without Markdown. Each starter must include a targetTerm from the personalized questions, copied exactly, so the learner can reuse those words when answering.',
...
function readingMessages(topic, grammarTopic, vocabularyItems, learnerProfile) {
  const { terms } = readingVocabularyTerms(vocabularyItems);
  return [{
    role: 'system',
    content: [
      ...
      'All student-facing content must be natural English at the target CEFR level and directly connected to the supplied lesson topic.',
      ...
      'The supplied Grammar topic is authoritative. Naturally include 4 to 7 clear and accurate examples of that target grammar in the reading text, without explaining the grammar.',
      'Select 5 to 6 distinct entries from the supplied Target Vocabulary. Copy each selected entry exactly into usedVocabularyTerms and include that exact text naturally in the reading text.',
      ...
      'Both image prompts must be in English and describe modern educational visuals suitable for teenagers, explicitly requiring no text or readable text and avoiding childish or preschool aesthetics.',
...
function listeningMessages(topic, grammarTopic, vocabularyItems, learnerProfile) {
  const { terms } = readingVocabularyTerms(vocabularyItems);
  return [{
    role: 'system',
    content: [
      ...
      'The supplied Grammar topic is authoritative. Naturally include 4 to 7 clear and accurate examples of that target grammar in the audio script, without explaining the grammar.',
      'Naturally include 5 to 6 distinct entries from the supplied Target Vocabulary, copying their wording exactly.',
...
function grammarPresentationMessages(topic, grammarTopic, learnerProfile) {
  return [{
    role: 'system',
    content: [
      learnerProfileInstructions(learnerProfile),
      ...
      'All generated copy must be in natural English at the target CEFR level.',
      ...
      'Create one to three Quick Rule sections. Use concise Markdown bullet points to cover meaning, affirmative form, and relevant negative/question forms, with an example at the target CEFR level where useful.',
...
function grammarFocusMessages(topic, grammarTopic, vocabularyItems, learnerProfile) {
  const { terms } = readingVocabularyTerms(vocabularyItems);
  return [{
    role: 'system',
    content: [
      ...
      'All generated lesson copy, including Teacher’s Notes, must be in English at the target CEFR level. The image prompt must be in English.',
      ...
      'Create one connected dialogue of six to eight turns for Task 2. Put every turn on a separate line in task2Dialogue, so the parsed string contains a newline (\\n) between consecutive turns. Start every line with a speaker label formatted as **Name:** and place exactly nine literal {{gap}} markers in reading order.',
      ...
      'Create a topic-related miniSituation prompt that elicits three to five uses of the target grammar and an English modern educational imagePrompt suitable for teenagers that explicitly requires no text and avoids childish or preschool aesthetics.',
...
function guidedSpeakingMessages(topic, vocabularyItems, learnerProfile) {
  const { terms } = readingVocabularyTerms(vocabularyItems);
  return [{
    role: 'system',
    content: [
      learnerProfileInstructions(learnerProfile),
      'You design one-to-one English lessons.',
      'Generate all variable content for an eight-minute Guided Speaking role-play in the existing template.',
      'Use the Lesson topic as one coherent practical situation in which the learner and teacher must negotiate and reach one shared decision.',
      'All generated content must be natural English at the target CEFR level. Do not use Russian or any other language.',
      'Do not generate component titles, IDs, role titles, section titles, colors, icons, visibility settings, Read the instructions, or How to Play; the application adds them statically.',
      'Select 2 to 4 distinct entries from Target Vocabulary. Copy each selected entry exactly into usedVocabularyTerms and use every selected entry naturally in the role cards or dialogue.',
      'Teacher Notes must be concise Markdown guidance covering how to start, support weaker learners, challenge stronger learners, what to watch for, what to do if the learner gets stuck, target vocabulary, correction timing, and measurable success.',
      'In teacherNotes, the only allowed Markdown formatting is unordered lists (- item), ordered lists (1. item), bold (**text**), and italic (*text*). Put every list item on its own line.',
      'Do not use Markdown headings (#, ##, ###), HTML, blockquotes, code, links, tables, or any other formatting. If a list item needs a label, make the label bold instead of using a heading.',
      'Create complementary Student and Teacher role cards. For each role provide exactly two wants, one thing to avoid, two private constraints or facts, two short secret-mission actions, and one shared goal.',
      'The student and teacher goal strings must be exactly the same. The wants, things to avoid, and private facts should create a solvable need to discuss, suggest, react, clarify, agree or disagree, and compromise.',
      'For every Speaking Support category create exactly three short phrases suitable for the generated situation. Keep clarification phrases generally reusable and make other examples topic-relevant where natural.',
      'Create a coherent Example Dialogue of 6 to 10 alternating Teacher and Student turns. It must reflect the role-card information without exposing private facts unnaturally, demonstrate useful support phrases and selected vocabulary, and finish with the shared decision.',
      'Do not turn the activity into a grammar exercise and do not assume a grammar target.',
      'Return JSON matching the supplied schema and nothing else.',
    ].join('\n'),
  }, {
    role: 'user',
...
function wrapUpMessages(topic, grammarTopic, vocabularyItems, learnerProfile) {
  const { terms } = readingVocabularyTerms(vocabularyItems);
  return [{
    role: 'system',
    content: [
      learnerProfileInstructions(learnerProfile),
      'You design one-to-one English lessons.',
      'Generate only the variable content for a three-minute Wrap-Up in the existing 3–2–1 template.',
      'Use the Lesson topic, Grammar topic, and exact Target Vocabulary to review the lesson coherently.',
      'All generated copy, including Teacher’s Notes, must be natural English at the target CEFR level. Do not use Russian or any other language.',
      'Do not generate component titles, IDs, labels, colors, icons, visibility settings, numbering, or Self-assessment; the application adds them statically.',
      'Teacher’s Notes must contain three concise parts: measurable signs of success, specific support if the learner struggles, and a ready-to-say positive ending.',
      'The threePrompt must ask the learner to recall three words or phrases from the supplied Target Vocabulary in connection with the lesson topic.',
      'The twoPrompt must ask for two topic-related sentences that accurately use the supplied Grammar topic.',
      'Create exactly two concrete twoCues. Each cue must give a different topic-related idea for one sentence and make the intended grammar use clear without supplying the complete answer.',
      'The onePrompt must be one practical can-do question about the lesson topic that invites a short personal answer and use of the target grammar or vocabulary.',
      'Create exactly three short Possible language supports. Make them useful for answering the two and one steps, and naturally include the target grammar and relevant supplied vocabulary where possible.',
      'Do not use HTML. Use Markdown only when natural emphasis is useful inside prompts or cues.',
      'Return JSON matching the supplied schema and nothing else.',
    ].join('\n'),
  }, {
    role: 'user',
...
async function generateStructuredSection({
  ...
  if (!LESSON_MODEL_OPTIONS[model]) throw new Error('Модель генерации не задана или не поддерживается.');
  ...
  try {
    ...
    throw Object.assign(new Error(`Модель генерации вернула некорректный JSON для ${sectionName}: ${cause.message}`), { streamed });
  }
  validate(generated);
...
async function generateWarmUp({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: warmUpMessages(topic, { ageGroup, level }),
...
async function generateLessonMetadata({
  topic,
  ageGroup,
  level,
  apiKey,
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: lessonMetadataMessages(topic, { ageGroup, level }),
    schemaName: 'easyclass_lesson_metadata',
    schema: LESSON_METADATA_RESPONSE_SCHEMA,
    validate: buildLessonMetadata,
    sectionName: 'Lesson Metadata',
  });
}
...
async function generateLeadIn({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: leadInMessages(topic, { ageGroup, level }),
...
async function generateTargetVocabulary({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: targetVocabularyMessages(topic, { ageGroup, level }),
...
async function generateReading({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: readingMessages(topic, grammarTopic, vocabularyItems, { ageGroup, level }),
...
async function generateListening({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: listeningMessages(topic, grammarTopic, vocabularyItems, { ageGroup, level }),
...
async function generateGrammarPresentation({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: grammarPresentationMessages(topic, grammarTopic, { ageGroup, level }),
...
async function generateGrammarFocus({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: grammarFocusMessages(topic, grammarTopic, vocabularyItems, { ageGroup, level }),
...
async function generateGuidedSpeaking({
  topic,
  vocabularyItems,
  ageGroup,
  level,
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: guidedSpeakingMessages(topic, vocabularyItems, { ageGroup, level }),
    schemaName: 'easyclass_guided_speaking',
    schema: GUIDED_SPEAKING_RESPONSE_SCHEMA,
    validate: generated => buildGuidedSpeakingContent(generated, vocabularyItems),
    sectionName: 'Guided Speaking',
  });
}
...
async function generateWrapUp({
  ...
  return generateStructuredSection({
    ...
    onDelta,
    fetchImpl,
    messages: wrapUpMessages(topic, grammarTopic, vocabularyItems, { ageGroup, level }),
    schemaName: 'easyclass_wrap_up',
    schema: WRAP_UP_RESPONSE_SCHEMA,
    validate: buildWrapUpContent,
    sectionName: 'Wrap-Up',
  });
}
