...
function buildLessonMetadata(generated) {
  ...
  if (!/\b(?:no|without)\s+(?:visible\s+|readable\s+)?text\b/i.test(coverImagePrompt)) {
    throw new Error('Промпт для обложки урока должен явно запрещать текст на изображении.');
  }
