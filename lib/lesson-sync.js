...
(() => {
  function initLessonSync(options) {
    ...
    function connectSync() {
      const roomId = roomInput.value.trim();
      if (!roomId) { setSyncStatus('Status: enter class code first.'); return; }
      if (connected) return;
      ...
      setSyncStatus(`Status: connecting to class "${roomId}" as ${role}...`);
      eventSource.onopen = () => {
        connected = true;
        updateUrlParams();
        setSyncStatus(`Status: online in class "${roomId}" as ${role}.`);
        updateTeacherReadonlyState();
    ...
    copyLinkBtn.addEventListener('click', async () => {
      const link = getStudentLink();
      if (!link) { setSyncStatus('Status: enter class code first.'); return; }
