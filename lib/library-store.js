...
function publishLesson(draftId, adminId, input, database, assetsDirectory) {
  ...
  try {
    ...
    const cover = textField(input.cover, 'Обложка', 300);
    // Cover choices are bundled images; arbitrary URLs and private draft paths are not accepted.
    if (!/^\/assets\/images\/[a-z0-9-]+\.(png|jpg|webp)$/i.test(cover)
      || !fs.existsSync(path.join(__dirname, '..', cover))) fail('Выберите обложку из списка.');
...
function findLibraryAsset(id, fileName, database) {
  return database.prepare(`SELECT a.data FROM library_assets a JOIN library_lessons l ON l.id = a.lesson_id
    WHERE a.lesson_id = ? AND a.file_name = ? AND l.is_published = 1 AND l.is_available = 1`).get(id, fileName)?.data;
}
