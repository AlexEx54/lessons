...
const WRAP_UP_SELF_ASSESSMENT = Object.freeze({
  type: 'selfAssessment',
...
const GRAMMAR_PRESENTATION_TEACHER_NOTE_TEXT = `In this section, we use a **Guided Discovery** approach. Please, do not explain the grammar rules right away. Let the student analyze the examples, answer the Concept-Checking Questions (CCQs), and figure out the patterns themselves. Task 2 is just a quick check to see if they grasp the basic concept before moving to deeper practice.
...
function createWarmUpTeacherNote() {
  return {
    type: 'teacherNote',
...
function createSyntheticLesson(topic, { template = 'template-1' } = {}) {
  if (template === 'template-2') {
    const { createTemplateTwoLesson } = require('./synthetic-template-two.js');
    return createTemplateTwoLesson(topic, STAGE_BLUEPRINTS, createWarmUpTeacherNote());
  }
  if (template !== 'template-1') throw new Error('Неизвестный шаблон урока.');
        ...
        ? [createWarmUpTeacherNote(), {
          ...
          items: [{
            ...
            options: [{
              ...
              imagePrompt: 'Modern square educational illustration for teenagers of a brave player facing blocky fantasy monsters at night in a video game world, dynamic lighting, no text.',
            }, {
