...
function sanitizeVideoCallDiagnostic(message) {
  ...
  for (const field of ['peerGeneration', 'operation', 'previousTrack', 'nextTrack', 'trackNumber', 'videoReadyState',
...
function createVideoCallSignaling({ server, database, attachUpgrade = true }) {
  ...
  webSocketServer.on('connection', socket => {
    ...
    socket.diagnosticWindowStartedAt = Date.now();
    ...
    socket.on('message', (raw, isBinary) => {
      ...
      if (message.type === 'diagnostic') {
        // Bound bursts, but retain diagnostics throughout a long lesson.
        if (Date.now() - socket.diagnosticWindowStartedAt >= 60_000) {
          socket.diagnosticCount = 0;
          socket.diagnosticWindowStartedAt = Date.now();
        }
