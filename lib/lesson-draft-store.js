...
function updateMarkdownCard({ id, ownerAdminId, componentId, title, text, sections }, database) {
  ...
  if (componentId.endsWith('-answer-key')
    && findComponentMatches(draft.content.stages, 'oddOneOut', componentId.slice(0, -'-answer-key'.length)).length) {
    const error = new Error('Измените ответы и пояснения в задании «Найди лишнее»: ключ обновляется автоматически.');
    error.statusCode = 409;
    throw error;
  }
