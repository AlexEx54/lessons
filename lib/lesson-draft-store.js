...
function updateDragWordsInText({ id, ownerAdminId, componentId, title, instruction, words, text }, database) {
  ...
  for (const stage of draft.content.stages) {
    ...
    for (const component of stage.content || []) {
      if (component?.type === 'dragWordsInText' && component.id === componentId) matches.push(component);
    }
  }
  if (matches.length === 0) {
...
function updateDropdownChoice({ id, ownerAdminId, componentId, title, instruction, text, choices }, database) {
  ...
  for (const stage of draft.content.stages) {
    ...
    for (const component of stage.content || []) {
      if (component?.type === 'dropdownChoice' && component.id === componentId) matches.push(component);
    }
  }
  if (matches.length === 0) {
...
function updateGapFill({ id, ownerAdminId, componentId, title, instruction, text, gaps }, database) {
  ...
  for (const stage of draft.content.stages) {
    ...
    for (const component of stage.content || []) {
      if (component?.type === 'gapFill' && component.id === componentId) matches.push(component);
    }
  }
  if (matches.length === 0) {
...
function updatePanelByType({
  ...
  for (const stage of draft.content.stages) {
    ...
    for (const component of stage.content || []) {
      ...
      if (componentType === 'illustratedTextPanel'
        && component?.type === 'miniSituation'
        && component.situation?.id === panelId) {
        matches.push(component.situation);
      }
    }
  }
  if (matches.length === 0) {
...
function findMiniSituationMatches(draft, componentId) {
  ...
  for (const stage of draft.content.stages) {
    ...
    for (const component of stage.content || []) {
      if (component?.type === 'miniSituation' && component.id === componentId) matches.push(component);
    }
  }
  if (matches.length === 0) {
