...
function updateSuggestedAnswers({ id, ownerAdminId, componentId, text }, database) {
  const normalizedText = typeof text === 'string' ? text.trim() : '';
  if (!normalizedText) {
    const error = new Error('Содержимое Suggested answers не может быть пустым.');
    error.statusCode = 400;
    throw error;
  }
  ...
  for (const stage of draft.content.stages) {
    ...
    for (const component of stage.content || []) {
      if (component?.type === 'suggestedAnswers' && component.id === componentId) matches.push(component);
    }
  }

  if (matches.length === 0) {
    const error = new Error('Suggested answers не найден.');
    error.statusCode = 404;
    throw error;
  }
  if (matches.length > 1) {
    const error = new Error('В черновике найдено несколько Suggested answers с одинаковым id.');
    error.statusCode = 409;
    throw error;
  }
