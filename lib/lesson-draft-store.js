...
function attachGeneration(draft, database) {
  ...
  return { ...draft, generation: generationSummary(findLessonGeneration(draft.id, database)) };
}

function createLessonDraft({ ownerAdminId, topic, template, content }, database) {
  ...
  database.prepare(`
    INSERT INTO lesson_drafts (
      id, owner_admin_id, topic, template_id, status, content_json, created_at, updated_at
    ) VALUES (?, ?, ?, ?, 'generating', ?, ?, ?)
  `).run(id, ownerAdminId, topic, template, content === undefined ? null : JSON.stringify(content), now, now);
