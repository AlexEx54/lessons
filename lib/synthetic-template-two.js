...
function createTemplateTwoLesson(topic, blueprints, teacherNote) {
  ...
  return {
    ...
    meta: { topic: String(topic || '').trim(), title: String(topic || '').trim(),
      level: 'A2', lessonNumber: 1, durationMinutes: 60, generatedBy: 'synthetic' },
    stages: blueprints.flatMap(stage => stage.id === 'target-vocabulary'
      ? [stage, { id: 'watch-and-interact', title: 'Watch & interact', subtitle: '', durationMinutes: 10, icon: 'video' }]
      : [stage]).map((stage, index) => ({ ...stage, number: index + 1,
      subtitle: {
        'warm-up': 'Find the Odd One Out!',
        'lead-in': 'Fact or Myth?',
        'target-vocabulary': 'Explore & Practise New Words',
        'watch-and-interact': 'Predict, Watch & Discuss',
      }[stage.id] || '',
      content: stage.id === 'warm-up' ? [teacherNote, exercise, createOddOneOutAnswerKey(exercise)] : stage.id === 'lead-in' ? createTemplateTwoLeadIn() : stage.id === 'target-vocabulary' ? createTemplateTwoTargetVocabulary() : stage.id === 'watch-and-interact' ? createTemplateTwoWatchAndInteract() : null,
    })),
  };
}
