...
function createTemplateTwoLesson(topic, teacherNote) {
  ...
  return {
    ...
    stages: TEMPLATE_2_STAGES.map((stage, index) => ({ ...stage, number: index + 1,
      content: stage.id === 'warm-up' ? [teacherNote, exercise, createOddOneOutAnswerKey(exercise)] : stage.id === 'lead-in' ? createTemplateTwoLeadIn() : stage.id === 'target-vocabulary' ? createTemplateTwoTargetVocabulary() : stage.id === 'watch-and-interact' ? createTemplateTwoWatchAndInteract() : stage.id === 'grammar-presentation' ? createTemplateTwoGrammarPresentation() : null,
    })),
  };
}
