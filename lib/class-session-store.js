...
const { studentComponent, applyComponentAction } = require('./class-component-handlers.js');
const supportedStages = new Set(['warm-up', 'lead-in']);
...
function sessionPayload(access, db) {
  ...
  return { role: access.role, state, availableStageIds: availableStageIds(lesson.content), lesson: {
    id: lesson.id,
    content: access.role === 'student' ? studentContent(lesson.content, state) : lesson.content,
  } };
}
...
function applyAction(access, action, db) {
  ...
  if (action.type === 'select-stage') {
    ...
    const component = stage?.content?.find(item => item.id === action.componentId);
