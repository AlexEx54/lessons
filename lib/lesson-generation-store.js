...
function retryLessonGeneration({ draftId, output }, database) {
  const now = new Date().toISOString();
  const result = database.prepare(`
    UPDATE lesson_draft_generations
    SET status = 'running', reasoning_text = '', output_text = ?,
        started_at = ?, completed_at = NULL, error_message = NULL
    WHERE draft_id = ? AND status = 'failed' AND mode = 'ai'
  `).run(output || '', now, draftId);
  if (result.changes === 0) {
    throw Object.assign(new Error('Повторный запуск доступен только для неудачной AI-генерации.'), {
      statusCode: 409,
    });
  }
  return findLessonGeneration(draftId, database);
