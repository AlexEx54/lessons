      ...
      switch (control.type) {
        ...
        case 'controlledInputPractice':
          ...
          (control.items || []).forEach((item, ii) => {
            ...
            if (!isNonEmptyString(item && item.baseVerb)) {
              pushWarning(warnings, `${field}.controls[${ci}].items[${ii}].baseVerb`, 'controlledInputPractice item should include baseVerb (verb first form) for the input placeholder.');
            }
          });
          break;
