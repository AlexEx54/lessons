...
const { authorizeClass, readState, applyAction } = require('./class-session-store.js');
function createClassSessionSignaling({ database }) {
  ...
  webSocketServer.on('connection', socket => {
    ...
    try { send(socket, { type: 'snapshot', state: readState(access.classId, database) }); }
    ...
    socket.on('message', (raw, binary) => {
      ...
      try {
        ...
        const state = applyAction(access, action, database);
        for (const client of room.values()) send(client, { type: 'action', action, state });
      } catch (error) {
        try { send(socket, { type: 'action-error', error: error.statusCode ? error.message : 'Некорректное действие.', state: readState(access.classId, database) }); }
