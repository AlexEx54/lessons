...
Поведение Vocabulary в живом классе, общая модель состояния и синхронизация
описаны в [Синхронизации Vocabulary](class-session-vocabulary.md). Описания локального
состояния ниже относятся к просмотру черновика.
...
- явно написанный веб-адрес (`https://example.com`, `www.example.com` или
  `example.com/path`) — автоматически отображается как внешняя ссылка;
...
Не используйте HTML, заголовки Markdown, именованные Markdown-ссылки вида
`[название](адрес)`, изображения, таблицы или вложенные списки. Для внешней заметки
не передавайте подпись, цвет или начальное состояние сворачивания: интерфейс добавляет
их самостоятельно, а заметка всегда открыта при первой отрисовке. Настройки
`titleColor` и `icon` относятся только к вложенным блокам.
...
для преподавателя. В живом классе доступных стадий показ синхронизируется с учеником
и сохраняется в состоянии класса.
...
`howToPlay` — самостоятельная зелёная карточка с правилами активности: иконка
геймпада, заголовок, пронумерованные шаги и опциональный Tip. Визуально она
совпадает с guide-блоком внутри `describeAndGuess` и используется для вступления
к свободной устной практике, например к стадии Guided Speaking.
...
```json
{
  ...
  "tip": "Use examples, actions and feelings."
}
```

Поля:

- `type` — строго `howToPlay`.
- `id` — обязательный уникальный lowercase kebab-case идентификатор.
- `title` — обязательный непустой заголовок без HTML и Markdown.
- `steps` — от 1 до 8 непустых строк без HTML и Markdown в порядке отображения.
- `tip` — опциональная непустая строка без HTML и Markdown. Пустое значение не
  передавайте: поле должно отсутствовать целиком. Если tip есть, интерфейс сам
  добавляет подпись `Tip:` и иконку лампочки.

Карточка не хранит состояние и не интерактивна; декоративные картинки (геймпад и
облачка диалога) интерфейс подставляет сам. В review-редакторе администратор может
...
Администратор может редактировать `title` и `script`, а также загружать, заменять
или удалять файл в уже объявленном слоте. `type` и `id` после генерации
неизменяемы. Полный текст транскрипции показывается независимо от наличия
аудиофайла; загрузка, замена и удаление файла доступны только в режиме
редактирования. Кнопка копирования находится снизу слева, файловые действия —
снизу справа. Кнопка `Показать` с eye-иконкой закреплена в правом верхнем углу
транскрипции; пока это визуальная заглушка без действия, зарезервированная для
будущего показа транскрипции ученику.
...
Не передавайте состояние выбора в JSON содержания урока. В черновике и библиотеке
оно локальное и независимо для каждой пары. В живом классе Warm Up выбор ученика
сохраняется отдельно от содержания и синхронизируется с наблюдающим преподавателем. В редакторе можно только загрузить, заменить или
...
`gapFill` показывает связный текст с текстовыми пропусками. Ученик вводит форму
слова или фразы. Компонент подходит для диалогов и Complete the gaps, когда нужна
не готовая опция, а самостоятельный ввод. В пропуске можно показать пример —
например первую форму глагола — как placeholder.
...
- `type` — строго `gapFill`.
- `studentVisibility` — необязательное `always` (поведение по умолчанию) или `controlled`. Во втором случае преподаватель открывает задание кнопкой «Показать»; скрытие сохраняет ответы, сброс стадии очищает ответы и видимость.
- `fieldSize` — необязательное `wide`: для ответов целым предложением. Поле в 1,5 раза шире обычного (315px, но не шире текста). Без флага поля остаются стандартной ширины.
- `id` — обязательный уникальный lowercase kebab-case идентификатор.
- `title` — обязательная непустая строка. Разрешён только `**жирный текст**`;
  выделенный фрагмент окрашивается в `accentColor`.
- `instruction` — обязательная непустая строка без HTML и Markdown.
- `text` — непустой текст с 1–12 маркерами вида `[[gap-id]]`. В обычном тексте
  разрешён только `**жирный текст**`, который окрашивается в `accentColor`.
  Маркеры выделения не могут пересекать пропуск или перевод строки.
  Одиночные переводы строки сохраняются, пустая строка создаёт новый абзац.
- `accentColor` — опциональный цвет выделенного жирного текста в формате
  `#RRGGBB`. По умолчанию используется почти чёрный `#17182D`. Placeholder
  в поле тусклый и выровнен по центру; от `accentColor` берётся только лёгкий оттенок.
- `gaps` — от 1 до 12 объектов с уникальными lowercase kebab-case `id` и
  обязательным `answer`. Опциональный `example` — короткий plain text,
  который показывается внутри пустого поля. Пустой example не передавайте.
  `example` может совпадать с `answer`.
- Каждый id из `gaps` должен встретиться в `text` ровно один раз, лишние или
  повторные маркеры запрещены. Одинаковый текст ответа или примера разрешён
  в разных пропусках.
- HTML и любой Markdown кроме описанного `**жирного текста**` в `title` и `text`
  запрещены. В `instruction`, `answer` и `example` Markdown запрещён полностью.

Во время ввода совпадение проверяется без учёта регистра, крайних и повторяющихся
пробелов, а также прямого и типографского апострофа. Верный ответ получает
зелёную галочку; неверный ответ никак не помечается, и поле остаётся доступным
для исправления. Ответы ученика не сохраняются в JSON.

Answer Key в этот компонент не встроен. Если ключ нужен, добавьте отдельный
`markdownCard` с `studentVisibility: "teacherOnly"`. Для двух колонок
нумерованных ответов используйте `layout: "columns"` и формат `**1.**`, потому
что markdown-список всегда начинается с единицы.

В review-редакторе администратор меняет заголовок, инструкцию и текст прямо в
карточке. Выделенный фрагмент можно превратить в пропуск: он становится
стартовым `example` и черновиком `answer`. Для каждого пропуска можно менять
ответ и пример или убрать пропуск. В режиме редактирования жирное выделение
...
`fillInBlanks` показывает небольшой перемешанный банк слов и нумерованные
предложения с одним текстовым пропуском в каждом. Компонент подходит для
самостоятельного воспроизведения слов и фраз из vocabulary без выпадающих
вариантов.
...
Перед предложениями компонент автоматически показывает значения `items[].answer`
в перемешанном inline-банке. Банк не является отдельным полем JSON и не
сохраняется; он пересобирается при каждой отрисовке. Это подсказка для ученика,
а Teacher Answer Key ниже строится из тех же ответов и показывается только
преподавателю.
...
помечены тускло-зелёным. В черновике состояние ответов локальное и не передаётся в JSON урока.
В живом классе Reading ответы сохраняются в состоянии класса и синхронизируются
между учеником и учителем; правила проверки общие с черновиком.
