...
для озвучки и кнопка копирования полного скрипта; загрузка, замена и удаление
файла доступны только в режиме редактирования. Если файл уже загружен, preview
показывает заголовок, плеер и полный текст транскрипции. В блоке транскрипции
также видна кнопка `Показать` с eye-иконкой — пока это визуальная заглушка без
действия, зарезервированная для будущего показа транскрипции ученику.
