...
- `id` — обязательный уникальный идентификатор компонента в lowercase kebab-case.
- `title` и `instruction` — обязательные непустые строки без HTML и Markdown.
- `text` — непустой обычный текст с 1–12 маркерами вида `[[choice-id]]`. Одиночные
  переводы строки сохраняются, пустая строка создаёт новый абзац.
...
- HTML и Markdown во всех видимых строках запрещены. Старое поле `segments` не
  поддерживается.
...
Поля `type`, `id` компонента и id существующих choices неизменны.
