...
## Ограничение входа сегментации (pipelineVersion 4)

Перед сегментацией каждый принятый кадр рисуется в переиспользуемый OffscreenCanvas размера выхода: максимум 960 px по ширине на desktop и 640 px на mobile, с сохранением пропорций. Для камеры 1280×720 desktop анализирует 960×540. Этот же кадр используется для композиции. Ограничение действует во всех worker transport; исходный трек камеры и режим без эффекта не меняются. Модель, пороги маски, сглаживание и алгоритм размытия не менялись. Время подготовки входит в stage segmentation; maskWidth/maskHeight показывают размер возвращённой маски.
